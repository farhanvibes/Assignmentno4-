...
const quot: string = 'A person who never made a mistake never tried anything new.';
const mesage: string = `${famous_person} once said, "${quote}"`;
console.log(mesage);
...
const nameStrippedWhitespace: string = nameWithWhitespace.trim();
...
const favriteNumber: number = 7; // Store the favorite number in a variable
const mgs: string = `My favorite number is ${favoriteNumber}.`; // Create a message using the favorite number
...
let names: string[] = ["Ali", "Bilal", "Chand", "Dilawar"];
...
let magicians: string[] = ["David ", " Houdini", " Jillette", "Tailor"];
...
// Define the array of magician's names
let magcians: string[] = ["Copperfield", "Harry", "Penn", "Teller"];
...
// Define the array of magician's names
const bestmagicians: string[] = [" Copperfield", " Houdini", " Jillette", "Teller"];
// Define the show_magicians function
function greatmagicians(names: string[]): void {
  for (const name of names) {
    console.log(name);
  }
}
...
function createCar(manufacturer: string, model: string, ...extras: { [key: string]: any }[]): object {
  const carInfo: { manufacturer: string; model: string; extras: { [key: string]: any }[] } = {
    manufacturer,
    model,
    extras,
  };
